...
export default function NavBarLoggedAuthSecond() {
    ...
    useEffect(() => {
        ...
        Array.from(menu[0].children).map((item, index: any) => {
            ...
            } else if (item.textContent.includes(t('dashboard')) && pathname === `/${locale}/portal` || pathname === "/messages" || pathname == `/${locale}/portaladministrador` || pathname == `/${locale}/admin/portalcolaborador`) {
                item.scrollIntoView({ behavior: 'smooth' })
                item.classList.add('navbar-logged-menu-item-active');
            } else {
            ...
            <HStack className='navbar-logged-conteiner-top'>
                ...
                <HStack className='navbar-logged-user-conteiner'>
                    ...
                    <CustomMenu menuitems={[{ label: t('perfil'), value: '/portal/perfil' }]} icon={
                        <AvatarCustom name={authsecond.currentUser?.displayName || ''} image={authsecond.currentUser?.photoURL || '/icons/avatar.svg'} />
                    } />
                </HStack>
            </HStack>
            <HStack className='navbar-logged-conteiner-bottom'>
                <VStack className='navbar-logged-welcome'>
                    <Heading className='navbar-logged-welcome-text'>{t('welcome', { name: authsecond.currentUser?.displayName?.split(' ')[0] || '' })}</Heading>
